...
final class NetworkClient: Client {
    ...
    func performRequest<T>(api: APIEndpoint, decodeTo: T.Type, completion: @escaping (Result<T, APIError>) -> Void) where T : Decodable {
        transport.send(endPoint: api) { result in
            ...
            case .success((let data, let response)):
                ...
                if let headerFields = response.allHeaderFields as? [String: String] {
                    let url = api.baseURL
                    _ = HTTPCookie.cookies(withResponseHeaderFields: headerFields, for: url)
                }
